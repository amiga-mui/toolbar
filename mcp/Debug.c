...
void SetupDebug(void)
{
  ...
  kprintf("** Toolbar.mcp v" LIB_REV_STRING " startup ***********************\n");
  kprintf("Initializing runtime debugging:\n");

	if(GetVar("toolbar.mcp.debug", var, sizeof(var), 0) > 0)
	{
		char *tok;
    char *debug = var;
	...
	kprintf("set debug classes/flags (env:toolbar.mcp.debug): %08x/%08x\n", debug_classes, debug_flags);
